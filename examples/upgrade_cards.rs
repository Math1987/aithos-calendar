//! Operator-only, repeatable 0.3 -> 0.4 migration. Runtime IAM cannot read keys.
...
async fn main() -> Result<(), lambda_http::Error> {
    ...
    if args.len() != 4 || !matches!(args[0].as_str(), "check" | "apply") {
        return Err("Usage: upgrade_cards check|apply TABLE API_ORIGIN REGISTRY_ORIGIN".into());
    }
    ...
    loop {
        ...
        for item in page.items() {
            ...
            if record.booking_page_url.is_none() || record.agent.live {
                continue;
            }
            println!(
                "{}: {} 0.3.0 -> 0.4.0 (identity preserved)",
                record.agent.id,
            ...
            let updated = registry.upgrade_to_live(&record, key, &args[2])?;
